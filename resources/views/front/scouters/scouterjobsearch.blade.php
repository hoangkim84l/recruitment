...
<div class="container">
	...
	<div class="padding-right">		
		...
		<ul class="job-list full">
			@foreach($jobs as $job)
			<li><a href="{{ URL::to('/scouters/tim-viec-lam/chi-tiet/'.$job->ids) }}" style="height: auto;width:80%;float:left;">
				<img src="/files/avatar/company/{{$job->logo_url}}" alt="{{$job->comName}}" title="{{$job->comName}}"style="    /* padding-top: 87px; */
																															margin-top: 40px;
																															width: 13%;
																															height: -3px;
																															margin-left: 10px;">
				<div class="job-list-content">
					<h4  style="overflow: hidden;text-overflow: ellipsis;white-space: pre;width:100%;height:28px;">{{$job->name}}<br/></h4>
					...
					<p  style="overflow: hidden;text-overflow: ellipsis;white-space: pre;width:100%;height:25px;">{{ strip_tags($job->description)}}</p>
				</div>
				</a>
				<span style="color:red;float:left; padding: 35px 0;display: inline-block;text-align:center;">
...
function loadJobs(data){
	var html = '';
	var getUrl = "{{ URL::to('/scouters/tim-viec-lam/chi-tiet/') }}";
	for(var i in data){
		html += '<li>';
		html += '<a href="'+getUrl+"/"+data[i].ids+'" style="height: auto;width:80%;float:left;">';
		html += '		<img src="/files/avatar/company/'+data[i].logo_url+'" alt="Embrace_it" title="Embrace_it" style="margin-top: 40px;width: 13%;height: -3px;margin-left: 10px;">';
		html += '		<div class="job-list-content">';
		html += '			<h4 style="overflow: hidden;text-overflow: ellipsis;white-space: pre;width:100%;height:28px;">'+data[i].name+' <br></h4>';
		...
		html += '				</div>';
		html += '			<p style="overflow: hidden;text-overflow: ellipsis;white-space: pre;width:100%;height:25px;">'+data[i].description+'</p>';
		html += '		</div>';
		html += '		</a>';
		html += '		<span style="color:red;float:left; padding: 35px 0;display: inline-block;text-align:center;">';
